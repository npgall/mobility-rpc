 ...
 */
public class PingMessageConverterTest {

    @Test
    public void testPingMessageConverter() {
        PingMessageConverter converter = new PingMessageConverter();
        UUID uuid = UUID.randomUUID();
        Ping input = new Ping(uuid, "foo");
        byte[] serialized = converter.convertToProtobuf(input);
        System.out.println("Serialized to: " + serialized.length + " bytes");
        Ping output = converter.convertFromProtobuf(serialized);
